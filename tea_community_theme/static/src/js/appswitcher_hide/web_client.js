                ...
                if (_.isEmpty($.bbq.getState(true))) {
                    return new Model("res.users").call("read", [session.uid, ["action_id"]]).then(function(result) {
                        ...
                        if(data.action_id) {
                            ...
                            self.toggle_app_switcher(false);
                            var first_menu_url = $("#sidebar").find("li:first-child").find("a:first").attr("href");
                            //self.menu.change_menu_section(self.menu.action_id_to_primary_menu_id(first_menu_id));
                            if(first_menu_url) {
                                window.location.href=first_menu_url;
                            }
                        }
                    });
                } else {
                ...
                if (!_.isEqual(self._current_state, stringstate)) {
                    ...
                    if (state.action || (state.model && (state.view_type || state.id))) {
                        ...
                        return self.action_manager.do_load_state(state, !!self._current_state).then(function () {
                            ...
                            self.toggle_app_switcher(false);
                        }).fail(self.toggle_app_switcher.bind(self, false));
